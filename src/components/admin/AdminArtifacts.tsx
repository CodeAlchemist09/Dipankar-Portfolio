        ...
        {artifacts.map((a) => (
          <AdminCard key={a.id}>
            <div className="flex items-start justify-between gap-3 mb-4">
              ...
              <button onClick={() => setArtifacts((p) => p.filter((x) => x.id !== a.id))} className="mt-6 cursor-pointer rounded-lg border border-white/10 p-2.5 text-mute hover:text-red-300 transition-colors">
                <Trash2 size={13} />
              </button>
            </div>
